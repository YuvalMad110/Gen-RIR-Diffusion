...
METRIC_EXTRACTORS = {
    ...
    'edt': lambda m: abs(m['edt']['error']) if not np.isnan(m['edt']['error']) else np.inf,
    'drr': lambda m: abs(m['drr']['error']) if not np.isnan(m['drr']['error']) else np.inf,
    'c50': lambda m: abs(m['c50']['error']) if not np.isnan(m['c50']['error']) else np.inf,
    'c80': lambda m: abs(m['c80']['error']) if not np.isnan(m['c80']['error']) else np.inf,
